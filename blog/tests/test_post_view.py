...
class PostViewTest(TestCase):
    ...
    def test_post_view_uses_correct_context(self):
        ...
        self.assertEqual(response.context['post'].content, self.content)
        self.assertEqual(response.context['post'].author, self._author)
        self.assertEqual(response.context['post'].category, self.category)
        self.assertEqual(response.context['post'].category.title, self.category.title)
        self.assertEqual(response.context['post'].category.slug, self.category.slug)
        self.assertEqual(response.context['post'].tag.all()[0], self.tag)
        self.assertEqual(response.context['post'].tag.all()[0].title, self.tag.title)
        self.assertEqual(response.context['post'].tag.all()[0].slug, self.tag.slug)
        self.assertEqual(response.context['post'].created_at.date(), timezone.now().date())
