...
class CategoryViewTests(TestCase):
    ...
    def setUp(self):
        ...
        for counter in range(0, self._tag_count):
            Tag.objects.create(
                title=f"Test Tag {counter}",
                slug=f"test-tag-{counter}",
            )

        for counter in range(1, self._post_count):
            post = Post.objects.get(slug=f"test-post-{counter}")
            if counter % 2:
                post.tag.add(Tag.objects.get(slug="test-tag-2"))
            else:
                post.tag.add(Tag.objects.get(slug="test-tag-1"))
                post.tag.add(Tag.objects.get(slug="test-tag-3"))
    ...
    def test_tag_exist_all_by_name(self):
        ...
        for counter in range(1, self._tag_count):
            response = self.client.get(
                reverse(
                    "tag",
                    kwargs={"slug": f"test-tag-{counter}"},
                )
            )
            self.assertEqual(response.status_code, 200)
        self.assertEqual(counter, self._tag_count - 1)
